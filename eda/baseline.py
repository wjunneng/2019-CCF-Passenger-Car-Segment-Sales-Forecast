...
def rule1(train_sales):
    train_sales = train_sales.set_index(["adcode", "bodyType", "model", "province", "regYear", "regMonth"])[
        ["salesVolume"]].unstack(level=-1)
    ...
    presale1 = train_sales[["adcode", "bodyType", "model", "province", 'pre1']].rename(
        columns={'pre1': 'forecastVolum1'})
    presale1['regMonth'] = 1
    presale2 = train_sales[["adcode", "bodyType", "model", "province", 'pre2']].rename(
        columns={'pre2': 'forecastVolum1'})
    presale2['regMonth'] = 2
    presale3 = train_sales[["adcode", "bodyType", "model", "province", 'pre3']].rename(
        columns={'pre3': 'forecastVolum1'})
    presale3['regMonth'] = 3
    presale4 = train_sales[["adcode", "bodyType", "model", "province", 'pre4']].rename(
        columns={'pre4': 'forecastVolum1'})
    presale4['regMonth'] = 4
...
def rule_2(train_sales):
    ...
    m1_07_volum = train_sales.loc[
                      (train_sales.regYear == 2017) & (train_sales.regMonth == 7), 'salesVolume'].values * m1_07
    ...
    evaluation_public.loc[evaluation_public.regMonth == 2, 'forecastVolum'] = evaluation_public.loc[(
                                                                                                            evaluation_public.regYear == 2018) & (
                                                                                                            evaluation_public.regMonth == 1), 'forecastVolum'].values / m1
    evaluation_public.loc[evaluation_public.regMonth == 3, 'forecastVolum'] = evaluation_public.loc[(
                                                                                                            evaluation_public.regYear == 2018) & (
                                                                                                            evaluation_public.regMonth == 2), 'forecastVolum'].values / m2
...
rule[['id', 'forecastVolum']].round().astype(int).to_csv('../data/submit/rule_submit.csv', index=False)
